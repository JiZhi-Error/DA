...
public final class j implements b {
    ...
    private boolean fS(boolean z) {
        ...
        Log.d("MicroMsg.SilkFileOperator", "Open file:" + this.file + " forWrite:" + z);
        try {
            this.file = s.dB(this.fileName, z);
            AppMethodBeat.o(130042);
            return true;
        } catch (Exception e2) {
            Log.e("MicroMsg.SilkFileOperator", "ERR: OpenFile[" + this.fileName + "] failed:[" + e2.getMessage() + "]");
            this.file = null;
            AppMethodBeat.o(130042);
            return false;
        }
    ...
    public final g dB(int i2, int i3) {
        ...
        if (i2 < 0 || i3 <= 0) {
            ...
            try {
                ...
                Log.d("MicroMsg.SilkFileOperator", "DBG: ReadFile[" + this.fileName + "] readOffset:" + i2 + " readRet:" + read + " fileNow:" + this.file.getFilePointer() + " fileSize:" + length);
                if (read < 0) {
                    read = 0;
                }
                gVar.dAc = read;
                ...
                Log.e("MicroMsg.SilkFileOperator", "ERR: ReadFile[" + this.fileName + "] Offset:" + i2 + "  failed:[" + e2.getMessage() + "] ");
                bhP();
                gVar.ret = -1;
                AppMethodBeat.o(130043);
            }
    ...
    public final int write(byte[] bArr, int i2, int i3) {
        ...
        if (this.file != null || fS(true)) {
            try {
                ...
                Log.e("MicroMsg.SilkFileOperator", "ERR: WriteFile[" + this.fileName + "] Offset:" + i3 + " failed:[" + e2.getMessage() + "]");
                bhP();
                AppMethodBeat.o(130044);
                return -3;
            }
