package com.tencent.mm.protocal.protobuf;

import com.tencent.matrix.trace.core.AppMethodBeat;
import com.tencent.mm.bw.a;
import g.a.a.b;
import java.util.LinkedList;

public final class drr extends a {
    public SKBuiltinBuffer_t KHp;
    public String LRO;
    public String Lir;
    public String Lis;
    public dqi LpA;
    public dqi LpB;
    public dqi Lqk;
    public int MUk;
    public dqi Mjj;
    public int MmK;
    public String MmL;
    public String MmM;
    public String MmN;
    public int MmO;
    public ebj MmQ;
    public aeq MmR;
    public int kdY;
    public String kdZ;
    public String kea;
    public String keb;
    public int kec;
    public String ked;
    public int kee;
    public int kef;
    public String keg;
    public String keh;
    public String kei;

    @Override // com.tencent.mm.bw.a
    public final int op(int i2, Object... objArr) {
        AppMethodBeat.i(152681);
        if (i2 == 0) {
            g.a.a.c.a aVar = (g.a.a.c.a) objArr[0];
            if (this.Lqk == null) {
                b bVar = new b("Not all required fields were included: UserName");
                AppMethodBeat.o(152681);
                throw bVar;
            } else if (this.Mjj == null) {
                b bVar2 = new b("Not all required fields were included: NickName");
                AppMethodBeat.o(152681);
                throw bVar2;
            } else if (this.LpA == null) {
                b bVar3 = new b("Not all required fields were included: PYInitial");
                AppMethodBeat.o(152681);
                throw bVar3;
            } else if (this.LpB == null) {
                b bVar4 = new b("Not all required fields were included: QuanPin");
                AppMethodBeat.o(152681);
                throw bVar4;
            } else if (this.KHp == null) {
                b bVar5 = new b("Not all required fields were included: ImgBuf");
                AppMethodBeat.o(152681);
                throw bVar5;
            } else {
                if (this.Lqk != null) {
                    aVar.ni(1, this.Lqk.computeSize());
                    this.Lqk.writeFields(aVar);
                }
                if (this.Mjj != null) {
                    aVar.ni(2, this.Mjj.computeSize());
                    this.Mjj.writeFields(aVar);
                }
                if (this.LpA != null) {
                    aVar.ni(3, this.LpA.computeSize());
                    this.LpA.writeFields(aVar);
                }
                if (this.LpB != null) {
                    aVar.ni(4, this.LpB.computeSize());
                    this.LpB.writeFields(aVar);
                }
                aVar.aM(5, this.kdY);
                if (this.KHp != null) {
                    aVar.ni(6, this.KHp.computeSize());
                    this.KHp.writeFields(aVar);
                }
                if (this.kdZ != null) {
                    aVar.e(7, this.kdZ);
                }
                if (this.kea != null) {
                    aVar.e(8, this.kea);
                }
                if (this.keb != null) {
                    aVar.e(9, this.keb);
                }
                aVar.aM(10, this.kec);
                aVar.aM(11, this.MmK);
                if (this.MmL != null) {
                    aVar.e(12, this.MmL);
                }
                if (this.MmM != null) {
                    aVar.e(13, this.MmM);
                }
                if (this.ked != null) {
                    aVar.e(14, this.ked);
                }
                if (this.MmN != null) {
                    aVar.e(15, this.MmN);
                }
                aVar.aM(16, this.MmO);
                aVar.aM(17, this.kef);
                aVar.aM(18, this.kee);
                if (this.keg != null) {
                    aVar.e(19, this.keg);
                }
                if (this.MmQ != null) {
                    aVar.ni(20, this.MmQ.computeSize());
                    this.MmQ.writeFields(aVar);
                }
                if (this.keh != null) {
                    aVar.e(21, this.keh);
                }
                if (this.kei != null) {
                    aVar.e(22, this.kei);
                }
                if (this.MmR != null) {
                    aVar.ni(23, this.MmR.computeSize());
                    this.MmR.writeFields(aVar);
                }
                if (this.Lir != null) {
                    aVar.e(24, this.Lir);
                }
                if (this.Lis != null) {
                    aVar.e(25, this.Lis);
                }
                if (this.LRO != null) {
                    aVar.e(26, this.LRO);
                }
                aVar.aM(27, this.MUk);
                AppMethodBeat.o(152681);
                return 0;
            }
        } else if (i2 == 1) {
            int nh = this.Lqk != null ? g.a.a.a.nh(1, this.Lqk.computeSize()) + 0 : 0;
            if (this.Mjj != null) {
                nh += g.a.a.a.nh(2, this.Mjj.computeSize());
            }
            if (this.LpA != null) {
                nh += g.a.a.a.nh(3, this.LpA.computeSize());
            }
            if (this.LpB != null) {
                nh += g.a.a.a.nh(4, this.LpB.computeSize());
            }
            int bu = nh + g.a.a.b.b.a.bu(5, this.kdY);
            if (this.KHp != null) {
                bu += g.a.a.a.nh(6, this.KHp.computeSize());
            }
            if (this.kdZ != null) {
                bu += g.a.a.b.b.a.f(7, this.kdZ);
            }
            if (this.kea != null) {
                bu += g.a.a.b.b.a.f(8, this.kea);
            }
            if (this.keb != null) {
                bu += g.a.a.b.b.a.f(9, this.keb);
            }
            int bu2 = bu + g.a.a.b.b.a.bu(10, this.kec) + g.a.a.b.b.a.bu(11, this.MmK);
            if (this.MmL != null) {
                bu2 += g.a.a.b.b.a.f(12, this.MmL);
            }
            if (this.MmM != null) {
                bu2 += g.a.a.b.b.a.f(13, this.MmM);
            }
            if (this.ked != null) {
                bu2 += g.a.a.b.b.a.f(14, this.ked);
            }
            if (this.MmN != null) {
                bu2 += g.a.a.b.b.a.f(15, this.MmN);
            }
            int bu3 = bu2 + g.a.a.b.b.a.bu(16, this.MmO) + g.a.a.b.b.a.bu(17, this.kef) + g.a.a.b.b.a.bu(18, this.kee);
            if (this.keg != null) {
                bu3 += g.a.a.b.b.a.f(19, this.keg);
            }
            if (this.MmQ != null) {
                bu3 += g.a.a.a.nh(20, this.MmQ.computeSize());
            }
            if (this.keh != null) {
                bu3 += g.a.a.b.b.a.f(21, this.keh);
            }
            if (this.kei != null) {
                bu3 += g.a.a.b.b.a.f(22, this.kei);
            }
            if (this.MmR != null) {
                bu3 += g.a.a.a.nh(23, this.MmR.computeSize());
            }
            if (this.Lir != null) {
                bu3 += g.a.a.b.b.a.f(24, this.Lir);
            }
            if (this.Lis != null) {
                bu3 += g.a.a.b.b.a.f(25, this.Lis);
            }
            if (this.LRO != null) {
                bu3 += g.a.a.b.b.a.f(26, this.LRO);
            }
            int bu4 = bu3 + g.a.a.b.b.a.bu(27, this.MUk);
            AppMethodBeat.o(152681);
            return bu4;
        } else if (i2 == 2) {
            g.a.a.a.a aVar2 = new g.a.a.a.a((byte[]) objArr[0], unknownTagHandler);
            for (int nextFieldNumber = a.getNextFieldNumber(aVar2); nextFieldNumber > 0; nextFieldNumber = a.getNextFieldNumber(aVar2)) {
                if (!super.populateBuilderWithField(aVar2, this, nextFieldNumber)) {
                    aVar2.hPl();
                }
            }
            if (this.Lqk == null) {
                b bVar6 = new b("Not all required fields were included: UserName");
                AppMethodBeat.o(152681);
                throw bVar6;
            } else if (this.Mjj == null) {
                b bVar7 = new b("Not all required fields were included: NickName");
                AppMethodBeat.o(152681);
                throw bVar7;
            } else if (this.LpA == null) {
                b bVar8 = new b("Not all required fields were included: PYInitial");
                AppMethodBeat.o(152681);
                throw bVar8;
            } else if (this.LpB == null) {
                b bVar9 = new b("Not all required fields were included: QuanPin");
                AppMethodBeat.o(152681);
                throw bVar9;
            } else if (this.KHp == null) {
                b bVar10 = new b("Not all required fields were included: ImgBuf");
                AppMethodBeat.o(152681);
                throw bVar10;
            } else {
                AppMethodBeat.o(152681);
                return 0;
            }
        } else if (i2 == 3) {
            g.a.a.a.a aVar3 = (g.a.a.a.a) objArr[0];
            drr drr = (drr) objArr[1];
            int intValue = ((Integer) objArr[2]).intValue();
            switch (intValue) {
                case 1:
                    LinkedList<byte[]> awh = aVar3.awh(intValue);
                    int size = awh.size();
                    for (int i3 = 0; i3 < size; i3++) {
                        dqi dqi = new dqi();
                        g.a.a.a.a aVar4 = new g.a.a.a.a(awh.get(i3), unknownTagHandler);
                        for (boolean z = true; z; z = dqi.populateBuilderWithField(aVar4, dqi, a.getNextFieldNumber(aVar4))) {
                        }
                        drr.Lqk = dqi;
                    }
                    AppMethodBeat.o(152681);
                    return 0;
                case 2:
                    LinkedList<byte[]> awh2 = aVar3.awh(intValue);
                    int size2 = awh2.size();
                    for (int i4 = 0; i4 < size2; i4++) {
                        dqi dqi2 = new dqi();
                        g.a.a.a.a aVar5 = new g.a.a.a.a(awh2.get(i4), unknownTagHandler);
                        for (boolean z2 = true; z2; z2 = dqi2.populateBuilderWithField(aVar5, dqi2, a.getNextFieldNumber(aVar5))) {
                        }
                        drr.Mjj = dqi2;
                    }
                    AppMethodBeat.o(152681);
                    return 0;
                case 3:
                    LinkedList<byte[]> awh3 = aVar3.awh(intValue);
                    int size3 = awh3.size();
                    for (int i5 = 0; i5 < size3; i5++) {
                        dqi dqi3 = new dqi();
                        g.a.a.a.a aVar6 = new g.a.a.a.a(awh3.get(i5), unknownTagHandler);
                        for (boolean z3 = true; z3; z3 = dqi3.populateBuilderWithField(aVar6, dqi3, a.getNextFieldNumber(aVar6))) {
                        }
                        drr.LpA = dqi3;
                    }
                    AppMethodBeat.o(152681);
                    return 0;
                case 4:
                    LinkedList<byte[]> awh4 = aVar3.awh(intValue);
                    int size4 = awh4.size();
                    for (int i6 = 0; i6 < size4; i6++) {
                        dqi dqi4 = new dqi();
                        g.a.a.a.a aVar7 = new g.a.a.a.a(awh4.get(i6), unknownTagHandler);
                        for (boolean z4 = true; z4; z4 = dqi4.populateBuilderWithField(aVar7, dqi4, a.getNextFieldNumber(aVar7))) {
                        }
                        drr.LpB = dqi4;
                    }
                    AppMethodBeat.o(152681);
                    return 0;
                case 5:
                    drr.kdY = aVar3.UbS.zi();
                    AppMethodBeat.o(152681);
                    return 0;
                case 6:
                    LinkedList<byte[]> awh5 = aVar3.awh(intValue);
                    int size5 = awh5.size();
                    for (int i7 = 0; i7 < size5; i7++) {
                        SKBuiltinBuffer_t sKBuiltinBuffer_t = new SKBuiltinBuffer_t();
                        g.a.a.a.a aVar8 = new g.a.a.a.a(awh5.get(i7), unknownTagHandler);
                        for (boolean z5 = true; z5; z5 = sKBuiltinBuffer_t.populateBuilderWithField(aVar8, sKBuiltinBuffer_t, a.getNextFieldNumber(aVar8))) {
                        }
                        drr.KHp = sKBuiltinBuffer_t;
                    }
                    AppMethodBeat.o(152681);
                    return 0;
                case 7:
                    drr.kdZ = aVar3.UbS.readString();
                    AppMethodBeat.o(152681);
                    return 0;
                case 8:
                    drr.kea = aVar3.UbS.readString();
                    AppMethodBeat.o(152681);
                    return 0;
                case 9:
                    drr.keb = aVar3.UbS.readString();
                    AppMethodBeat.o(152681);
                    return 0;
                case 10:
                    drr.kec = aVar3.UbS.zi();
                    AppMethodBeat.o(152681);
                    return 0;
                case 11:
                    drr.MmK = aVar3.UbS.zi();
                    AppMethodBeat.o(152681);
                    return 0;
                case 12:
                    drr.MmL = aVar3.UbS.readString();
                    AppMethodBeat.o(152681);
                    return 0;
                case 13:
                    drr.MmM = aVar3.UbS.readString();
                    AppMethodBeat.o(152681);
                    return 0;
                case 14:
                    drr.ked = aVar3.UbS.readString();
                    AppMethodBeat.o(152681);
                    return 0;
                case 15:
                    drr.MmN = aVar3.UbS.readString();
                    AppMethodBeat.o(152681);
                    return 0;
                case 16:
                    drr.MmO = aVar3.UbS.zi();
                    AppMethodBeat.o(152681);
                    return 0;
                case 17:
                    drr.kef = aVar3.UbS.zi();
                    AppMethodBeat.o(152681);
                    return 0;
                case 18:
                    drr.kee = aVar3.UbS.zi();
                    AppMethodBeat.o(152681);
                    return 0;
                case 19:
                    drr.keg = aVar3.UbS.readString();
                    AppMethodBeat.o(152681);
                    return 0;
                case 20:
                    LinkedList<byte[]> awh6 = aVar3.awh(intValue);
                    int size6 = awh6.size();
                    for (int i8 = 0; i8 < size6; i8++) {
                        ebj ebj = new ebj();
                        g.a.a.a.a aVar9 = new g.a.a.a.a(awh6.get(i8), unknownTagHandler);
                        for (boolean z6 = true; z6; z6 = ebj.populateBuilderWithField(aVar9, ebj, a.getNextFieldNumber(aVar9))) {
                        }
                        drr.MmQ = ebj;
                    }
                    AppMethodBeat.o(152681);
                    return 0;
                case 21:
                    drr.keh = aVar3.UbS.readString();
                    AppMethodBeat.o(152681);
                    return 0;
                case 22:
                    drr.kei = aVar3.UbS.readString();
                    AppMethodBeat.o(152681);
                    return 0;
                case 23:
                    LinkedList<byte[]> awh7 = aVar3.awh(intValue);
                    int size7 = awh7.size();
                    for (int i9 = 0; i9 < size7; i9++) {
                        aeq aeq = new aeq();
                        g.a.a.a.a aVar10 = new g.a.a.a.a(awh7.get(i9), unknownTagHandler);
                        for (boolean z7 = true; z7; z7 = aeq.populateBuilderWithField(aVar10, aeq, a.getNextFieldNumber(aVar10))) {
                        }
                        drr.MmR = aeq;
                    }
                    AppMethodBeat.o(152681);
                    return 0;
                case 24:
                    drr.Lir = aVar3.UbS.readString();
                    AppMethodBeat.o(152681);
                    return 0;
                case 25:
                    drr.Lis = aVar3.UbS.readString();
                    AppMethodBeat.o(152681);
                    return 0;
                case 26:
                    drr.LRO = aVar3.UbS.readString();
                    AppMethodBeat.o(152681);
                    return 0;
                case 27:
                    drr.MUk = aVar3.UbS.zi();
                    AppMethodBeat.o(152681);
                    return 0;
                default:
                    AppMethodBeat.o(152681);
                    return -1;
            }
        } else {
            AppMethodBeat.o(152681);
            return -1;
        }
    }
}
