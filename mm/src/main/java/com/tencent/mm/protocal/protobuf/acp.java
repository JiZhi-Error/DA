package com.tencent.mm.protocal.protobuf;

import com.tencent.matrix.trace.core.AppMethodBeat;
import com.tencent.mm.bw.a;
import g.a.a.b;

public final class acp extends a {
    public long Lns;
    public int label;
    public String path;

    @Override // com.tencent.mm.bw.a
    public final int op(int i2, Object... objArr) {
        AppMethodBeat.i(201292);
        if (i2 == 0) {
            g.a.a.c.a aVar = (g.a.a.c.a) objArr[0];
            if (this.path == null) {
                b bVar = new b("Not all required fields were included: path");
                AppMethodBeat.o(201292);
                throw bVar;
            }
            if (this.path != null) {
                aVar.e(1, this.path);
            }
            aVar.aM(2, this.label);
            aVar.bb(3, this.Lns);
            AppMethodBeat.o(201292);
            return 0;
        } else if (i2 == 1) {
            int f2 = (this.path != null ? g.a.a.b.b.a.f(1, this.path) + 0 : 0) + g.a.a.b.b.a.bu(2, this.label) + g.a.a.b.b.a.r(3, this.Lns);
            AppMethodBeat.o(201292);
            return f2;
        } else if (i2 == 2) {
            g.a.a.a.a aVar2 = new g.a.a.a.a((byte[]) objArr[0], unknownTagHandler);
            for (int nextFieldNumber = a.getNextFieldNumber(aVar2); nextFieldNumber > 0; nextFieldNumber = a.getNextFieldNumber(aVar2)) {
                if (!super.populateBuilderWithField(aVar2, this, nextFieldNumber)) {
                    aVar2.hPl();
                }
            }
            if (this.path == null) {
                b bVar2 = new b("Not all required fields were included: path");
                AppMethodBeat.o(201292);
                throw bVar2;
            }
            AppMethodBeat.o(201292);
            return 0;
        } else if (i2 == 3) {
            g.a.a.a.a aVar3 = (g.a.a.a.a) objArr[0];
            acp acp = (acp) objArr[1];
            switch (((Integer) objArr[2]).intValue()) {
                case 1:
                    acp.path = aVar3.UbS.readString();
                    AppMethodBeat.o(201292);
                    return 0;
                case 2:
                    acp.label = aVar3.UbS.zi();
                    AppMethodBeat.o(201292);
                    return 0;
                case 3:
                    acp.Lns = aVar3.UbS.zl();
                    AppMethodBeat.o(201292);
                    return 0;
                default:
                    AppMethodBeat.o(201292);
                    return -1;
            }
        } else {
            AppMethodBeat.o(201292);
            return -1;
        }
    }
}
