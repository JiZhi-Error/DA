package com.tencent.mm.protocal.protobuf;

import com.tencent.matrix.trace.core.AppMethodBeat;
import com.tencent.mm.bw.a;
import g.a.a.b;

public final class eyw extends a {
    public String MRZ;
    public long Nvw;
    public String oUJ;

    @Override // com.tencent.mm.bw.a
    public final int op(int i2, Object... objArr) {
        AppMethodBeat.i(32536);
        if (i2 == 0) {
            g.a.a.c.a aVar = (g.a.a.c.a) objArr[0];
            if (this.MRZ == null) {
                b bVar = new b("Not all required fields were included: Talker");
                AppMethodBeat.o(32536);
                throw bVar;
            } else if (this.oUJ == null) {
                b bVar2 = new b("Not all required fields were included: NickName");
                AppMethodBeat.o(32536);
                throw bVar2;
            } else {
                if (this.MRZ != null) {
                    aVar.e(1, this.MRZ);
                }
                if (this.oUJ != null) {
                    aVar.e(2, this.oUJ);
                }
                aVar.bb(3, this.Nvw);
                AppMethodBeat.o(32536);
                return 0;
            }
        } else if (i2 == 1) {
            int f2 = this.MRZ != null ? g.a.a.b.b.a.f(1, this.MRZ) + 0 : 0;
            if (this.oUJ != null) {
                f2 += g.a.a.b.b.a.f(2, this.oUJ);
            }
            int r = f2 + g.a.a.b.b.a.r(3, this.Nvw);
            AppMethodBeat.o(32536);
            return r;
        } else if (i2 == 2) {
            g.a.a.a.a aVar2 = new g.a.a.a.a((byte[]) objArr[0], unknownTagHandler);
            for (int nextFieldNumber = a.getNextFieldNumber(aVar2); nextFieldNumber > 0; nextFieldNumber = a.getNextFieldNumber(aVar2)) {
                if (!super.populateBuilderWithField(aVar2, this, nextFieldNumber)) {
                    aVar2.hPl();
                }
            }
            if (this.MRZ == null) {
                b bVar3 = new b("Not all required fields were included: Talker");
                AppMethodBeat.o(32536);
                throw bVar3;
            } else if (this.oUJ == null) {
                b bVar4 = new b("Not all required fields were included: NickName");
                AppMethodBeat.o(32536);
                throw bVar4;
            } else {
                AppMethodBeat.o(32536);
                return 0;
            }
        } else if (i2 == 3) {
            g.a.a.a.a aVar3 = (g.a.a.a.a) objArr[0];
            eyw eyw = (eyw) objArr[1];
            switch (((Integer) objArr[2]).intValue()) {
                case 1:
                    eyw.MRZ = aVar3.UbS.readString();
                    AppMethodBeat.o(32536);
                    return 0;
                case 2:
                    eyw.oUJ = aVar3.UbS.readString();
                    AppMethodBeat.o(32536);
                    return 0;
                case 3:
                    eyw.Nvw = aVar3.UbS.zl();
                    AppMethodBeat.o(32536);
                    return 0;
                default:
                    AppMethodBeat.o(32536);
                    return -1;
            }
        } else {
            AppMethodBeat.o(32536);
            return -1;
        }
    }
}
