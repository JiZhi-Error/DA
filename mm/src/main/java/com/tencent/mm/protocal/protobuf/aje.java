package com.tencent.mm.protocal.protobuf;

import com.tencent.matrix.trace.core.AppMethodBeat;
import com.tencent.mm.bw.a;

public final class aje extends a {
    public String IconUrl;
    public String LsI;
    public int LsJ;
    public String Name;

    @Override // com.tencent.mm.bw.a
    public final int op(int i2, Object... objArr) {
        AppMethodBeat.i(104767);
        if (i2 == 0) {
            g.a.a.c.a aVar = (g.a.a.c.a) objArr[0];
            if (this.Name != null) {
                aVar.e(1, this.Name);
            }
            if (this.IconUrl != null) {
                aVar.e(2, this.IconUrl);
            }
            if (this.LsI != null) {
                aVar.e(3, this.LsI);
            }
            aVar.aM(4, this.LsJ);
            AppMethodBeat.o(104767);
            return 0;
        } else if (i2 == 1) {
            int f2 = this.Name != null ? g.a.a.b.b.a.f(1, this.Name) + 0 : 0;
            if (this.IconUrl != null) {
                f2 += g.a.a.b.b.a.f(2, this.IconUrl);
            }
            if (this.LsI != null) {
                f2 += g.a.a.b.b.a.f(3, this.LsI);
            }
            int bu = f2 + g.a.a.b.b.a.bu(4, this.LsJ);
            AppMethodBeat.o(104767);
            return bu;
        } else if (i2 == 2) {
            g.a.a.a.a aVar2 = new g.a.a.a.a((byte[]) objArr[0], unknownTagHandler);
            for (int nextFieldNumber = a.getNextFieldNumber(aVar2); nextFieldNumber > 0; nextFieldNumber = a.getNextFieldNumber(aVar2)) {
                if (!super.populateBuilderWithField(aVar2, this, nextFieldNumber)) {
                    aVar2.hPl();
                }
            }
            AppMethodBeat.o(104767);
            return 0;
        } else if (i2 == 3) {
            g.a.a.a.a aVar3 = (g.a.a.a.a) objArr[0];
            aje aje = (aje) objArr[1];
            switch (((Integer) objArr[2]).intValue()) {
                case 1:
                    aje.Name = aVar3.UbS.readString();
                    AppMethodBeat.o(104767);
                    return 0;
                case 2:
                    aje.IconUrl = aVar3.UbS.readString();
                    AppMethodBeat.o(104767);
                    return 0;
                case 3:
                    aje.LsI = aVar3.UbS.readString();
                    AppMethodBeat.o(104767);
                    return 0;
                case 4:
                    aje.LsJ = aVar3.UbS.zi();
                    AppMethodBeat.o(104767);
                    return 0;
                default:
                    AppMethodBeat.o(104767);
                    return -1;
            }
        } else {
            AppMethodBeat.o(104767);
            return -1;
        }
    }
}
