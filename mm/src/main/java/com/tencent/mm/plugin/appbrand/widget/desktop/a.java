...
public final class a {
    ...
    public static void init(Context context) {
        ...
        Log.i("MicroMsg.AppBrandDesktopConfig", "alvinluo DesktopConfig init mCurrentType: %d", Integer.valueOf(omD));
        if (omD == 1) {
            omE = 4.0f;
            omF = 4;
            omG = 3.0f;
        ...
        int c2 = b.c(context, omJ);
        if (omD == 1) {
            c2 = (int) (((float) c2) + (((float) (context.getResources().getDimensionPixelSize(R.dimen.k5) * 2)) * b.ez(context)));
        }
        if (c2 <= com.tencent.mm.cb.a.fromDPToPix(context, 10)) {
            omJ = omG;
            omK = omH;
        } else {
            omJ = omE;
        ...
        omL = max;
        omL = Math.min(max, 4);
        Log.i("MicroMsg.AppBrandDesktopConfig", "alvinluo DesktopConfig init showCountPerPage: %f, completelyShowCountPerPage: %d, mRecentAppBrandMaxShowLines: %d", Float.valueOf(omJ), Integer.valueOf(omK), Integer.valueOf(omL));
