package com.tencent.mm.plugin.game.protobuf;

import com.tencent.matrix.trace.core.AppMethodBeat;
import com.tencent.mm.bw.a;
import g.a.a.b;
import java.util.LinkedList;

public final class ap extends a {
    public String Desc;
    public g xIz;
    public ar xKB;
    public aq xKC;

    @Override // com.tencent.mm.bw.a
    public final int op(int i2, Object... objArr) {
        AppMethodBeat.i(41733);
        if (i2 == 0) {
            g.a.a.c.a aVar = (g.a.a.c.a) objArr[0];
            if (this.xIz == null) {
                b bVar = new b("Not all required fields were included: AppItem");
                AppMethodBeat.o(41733);
                throw bVar;
            }
            if (this.xIz != null) {
                aVar.ni(1, this.xIz.computeSize());
                this.xIz.writeFields(aVar);
            }
            if (this.xKB != null) {
                aVar.ni(2, this.xKB.computeSize());
                this.xKB.writeFields(aVar);
            }
            if (this.xKC != null) {
                aVar.ni(3, this.xKC.computeSize());
                this.xKC.writeFields(aVar);
            }
            if (this.Desc != null) {
                aVar.e(4, this.Desc);
            }
            AppMethodBeat.o(41733);
            return 0;
        } else if (i2 == 1) {
            int nh = this.xIz != null ? g.a.a.a.nh(1, this.xIz.computeSize()) + 0 : 0;
            if (this.xKB != null) {
                nh += g.a.a.a.nh(2, this.xKB.computeSize());
            }
            if (this.xKC != null) {
                nh += g.a.a.a.nh(3, this.xKC.computeSize());
            }
            if (this.Desc != null) {
                nh += g.a.a.b.b.a.f(4, this.Desc);
            }
            AppMethodBeat.o(41733);
            return nh;
        } else if (i2 == 2) {
            g.a.a.a.a aVar2 = new g.a.a.a.a((byte[]) objArr[0], unknownTagHandler);
            for (int nextFieldNumber = a.getNextFieldNumber(aVar2); nextFieldNumber > 0; nextFieldNumber = a.getNextFieldNumber(aVar2)) {
                if (!super.populateBuilderWithField(aVar2, this, nextFieldNumber)) {
                    aVar2.hPl();
                }
            }
            if (this.xIz == null) {
                b bVar2 = new b("Not all required fields were included: AppItem");
                AppMethodBeat.o(41733);
                throw bVar2;
            }
            AppMethodBeat.o(41733);
            return 0;
        } else if (i2 == 3) {
            g.a.a.a.a aVar3 = (g.a.a.a.a) objArr[0];
            ap apVar = (ap) objArr[1];
            int intValue = ((Integer) objArr[2]).intValue();
            switch (intValue) {
                case 1:
                    LinkedList<byte[]> awh = aVar3.awh(intValue);
                    int size = awh.size();
                    for (int i3 = 0; i3 < size; i3++) {
                        g gVar = new g();
                        g.a.a.a.a aVar4 = new g.a.a.a.a(awh.get(i3), unknownTagHandler);
                        for (boolean z = true; z; z = gVar.populateBuilderWithField(aVar4, gVar, a.getNextFieldNumber(aVar4))) {
                        }
                        apVar.xIz = gVar;
                    }
                    AppMethodBeat.o(41733);
                    return 0;
                case 2:
                    LinkedList<byte[]> awh2 = aVar3.awh(intValue);
                    int size2 = awh2.size();
                    for (int i4 = 0; i4 < size2; i4++) {
                        ar arVar = new ar();
                        g.a.a.a.a aVar5 = new g.a.a.a.a(awh2.get(i4), unknownTagHandler);
                        for (boolean z2 = true; z2; z2 = arVar.populateBuilderWithField(aVar5, arVar, a.getNextFieldNumber(aVar5))) {
                        }
                        apVar.xKB = arVar;
                    }
                    AppMethodBeat.o(41733);
                    return 0;
                case 3:
                    LinkedList<byte[]> awh3 = aVar3.awh(intValue);
                    int size3 = awh3.size();
                    for (int i5 = 0; i5 < size3; i5++) {
                        aq aqVar = new aq();
                        g.a.a.a.a aVar6 = new g.a.a.a.a(awh3.get(i5), unknownTagHandler);
                        for (boolean z3 = true; z3; z3 = aqVar.populateBuilderWithField(aVar6, aqVar, a.getNextFieldNumber(aVar6))) {
                        }
                        apVar.xKC = aqVar;
                    }
                    AppMethodBeat.o(41733);
                    return 0;
                case 4:
                    apVar.Desc = aVar3.UbS.readString();
                    AppMethodBeat.o(41733);
                    return 0;
                default:
                    AppMethodBeat.o(41733);
                    return -1;
            }
        } else {
            AppMethodBeat.o(41733);
            return -1;
        }
    }
}
