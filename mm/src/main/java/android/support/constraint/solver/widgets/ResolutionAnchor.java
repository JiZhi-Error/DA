        ...
        if (this.state != 1 && this.type != 4) {
            ...
            if (this.type == 1 && (this.target == null || this.target.state == 1)) {
                ...
                if (this.myAnchor.mType == ConstraintAnchor.Type.LEFT || this.myAnchor.mType == ConstraintAnchor.Type.RIGHT) {
                    f3 = f2 - ((float) this.myAnchor.mOwner.getWidth());
                    f4 = this.myAnchor.mOwner.mHorizontalBiasPercent;
                } else {
                    f3 = f2 - ((float) this.myAnchor.mOwner.getHeight());
                ...
                float f5 = (f3 - ((float) margin)) - ((float) margin2);
                if (z) {
                    this.opposite.resolvedOffset = ((float) margin2) + this.opposite.target.resolvedOffset + (f5 * f4);
                    this.resolvedOffset = (this.target.resolvedOffset - ((float) margin)) - ((1.0f - f4) * f5);
                } else {
                    this.resolvedOffset = this.target.resolvedOffset + ((float) margin) + (f5 * f4);
                    this.opposite.resolvedOffset = (this.opposite.target.resolvedOffset - ((float) margin2)) - ((1.0f - f4) * f5);
                }
                didResolve();
