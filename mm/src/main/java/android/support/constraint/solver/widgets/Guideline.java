...
public class Guideline extends ConstraintWidget {
        ...
        if (parent != null) {
            if (getOrientation() == 1) {
                ...
                if (this.mRelativeBegin != -1) {
                    ...
                    int i3 = (int) (((float) parent.mWidth) * this.mRelativePercent);
                ...
                if (this.mRelativeBegin != -1) {
                    ...
                    int i4 = (int) (((float) parent.mHeight) * this.mRelativePercent);
    ...
    public void addToSolver(LinearSystem linearSystem) {
        ...
        if (constraintWidgetContainer != null) {
            ...
            boolean z2 = this.mParent != null ? this.mParent.mListDimensionBehaviors[0] == ConstraintWidget.DimensionBehaviour.WRAP_CONTENT : false;
            if (this.mOrientation == 0) {
                ConstraintAnchor anchor3 = constraintWidgetContainer.getAnchor(ConstraintAnchor.Type.TOP);
                constraintAnchor = constraintWidgetContainer.getAnchor(ConstraintAnchor.Type.BOTTOM);
                if (this.mParent == null) {
                    z = false;
                } else if (this.mParent.mListDimensionBehaviors[1] != ConstraintWidget.DimensionBehaviour.WRAP_CONTENT) {
                    z = false;
                }
                anchor = anchor3;
            } else {
            ...
            if (this.mRelativeBegin != -1) {
                ...
                if (z) {
                    linearSystem.addGreaterThan(linearSystem.createObjectVariable(constraintAnchor), createObjectVariable, 0, 5);
                }
            } else if (this.mRelativeEnd != -1) {
                ...
                linearSystem.addConstraint(LinearSystem.createRowDimensionPercent(linearSystem, linearSystem.createObjectVariable(this.mAnchor), linearSystem.createObjectVariable(anchor), linearSystem.createObjectVariable(constraintAnchor), this.mRelativePercent, this.mIsPositionRelaxed));
            }
        }
    }
    ...
    public void setDrawOrigin(int i2, int i3) {
        if (this.mOrientation == 1) {
            ...
            if (this.mRelativeBegin != -1) {
                ...
                setGuidePercent(((float) i4) / ((float) getParent().getWidth()));
            }
        } else {
            ...
            if (this.mRelativeBegin != -1) {
                ...
                setGuidePercent(((float) i5) / ((float) getParent().getHeight()));
            }
        }
    }
